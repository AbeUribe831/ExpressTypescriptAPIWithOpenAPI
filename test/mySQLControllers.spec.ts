...
const singleComment: SQLComment = {
  id: 1,
  postId: 1,
  name: "bob",
  email: "bob@bobbyb.com",
...
describe("unit testing all controller methods for /sql-comments route in mySQLControllers.ts", () => {
  it("getAllComments success", async () => {
    ...
    const value = await mySQLController.getAllComments(contextWithParamId);
    chai.assert.equal(comments, value.comments);
  });
  it("getAllComments throws an error", async () => {
    sandbox
      .stub(mySQLController, "getAllComments")
      .rejects(new Error("some internal error"));
    try {
      chai
        .expect(await mySQLController.getAllComments("not needed" as any))
        .to.throw();
    } catch (err) {
  ...
  it("getSingleComment success", async () => {
    sandbox
      .stub(mySQLController, "getSingleComment")
      .resolves({ comment: singleComment });

    const resComment = await mySQLController.getSingleComment(
      contextWithParamId
    );
  ...
  it("getSingleComment throws an error", async () => {
    ...
    try {
      chai
        .expect(await mySQLController.getSingleComment(contextWithParamId))
        .to.throw();
    } catch (err) {
      if (err instanceof Error)
        chai.assert.equal(err.message, "some internal error");
  ...
  it("postSingleComment success and returns one SQLComment", async () => {
    const mockReturnComment = {
      ...testBody,
      id: contextWithParamId.params.path.id,
    } as SQLComment;
    sandbox
      .stub(mySQLController, "postSingleComment")
      .resolves(mockReturnComment);
    contextWithParamId.req.body = testBody;
    const resComment = await mySQLController.postSingleComment(
      contextWithParamId
    );
  ...
  it("postSingleComment throws an error", async () => {
    ...
    try {
      chai
        .expect(await mySQLController.postSingleComment(contextWithParamId))
        .to.throw();
    } catch (err) {
      if (err instanceof Error)
        chai.assert.equal(err.message, "some internal error");
...
describe("integration testing all controller methods for /sql-comments route in mySQLControllers.ts", () => {
  it("getAllComments success", async () => {
    ...
    const queriedComments = await mySQLController.getAllComments(
      contextWithParamId
    );
  ...
  it("getAllComments error from service method getAllSQLComments", async () => {
    ...
    try {
      chai
        .expect(await mySQLController.getAllComments(contextWithParamId))
        .to.throw();
    } catch (e) {
  ...
  it("getSingleComment success", async () => {
    ...
    const resultComment = await mySQLController.getSingleComment(
      contextWithParamId
    );
  ...
  it("postSingleComment success", async () => {
    ...
    const resultCommnet = await mySQLController.postSingleComment(
      contextWithParamId
    );
